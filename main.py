...
def do_move_and_check_energy_and_accept_or_reject(geometry, dMin, dMax, T):
    ...
    #TOFU:Tidy up as this is a bit messy, this should be included in the move methods, so that the move method returns tmpGeometryData
    #update new positions to generate a neighbouring configuration
    tmpGeometryData = deepcopy(geometry.curve_object.data)
    if geometry.curve_object.geometryType == "ThreadedBeads":
        for (i,j) in indices[1:-1:]:
            tmpGeometryData[i][j]=newPos.pop(0)
    elif geometry.curve_object.geometryType == "Biarcs":
        for (i,j) in indices:
            tmpGeometryData[i][j]=newPos.pop(0)
